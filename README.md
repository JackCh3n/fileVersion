...
- 整理（clean）：把杂乱文件名规整为规范形态——去掉 Windows 复制计数 `(N)`，并把文件名里的日期 `YYYYMMDD`（或 `YYYY-MM-DD` 等）规整成 `V.YYYY_MM_DD`：
  ...
  - 已规整为 `名称V.日期` 的文件再 clean 不会叠加，只去掉残留的 `(N)`。
...
2. 选择：
   ...
   - **FileClean**：整理文件名——去掉 Windows 复制计数 `(N)`，并把文件名里的日期规整成 `V.YYYY_MM_DD`（如 `周例会相关工作汇报 - 20260604(1).docx` → `周例会相关工作汇报V.2026_06_04.docx`）；无日期的文件仅去掉 `(N)`（如 `信息安全自查(1).doc` → `信息安全自查.doc`）。

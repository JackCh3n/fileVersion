...
> **识别位置不限行尾**：即使版本号不在文件名最末尾也能正确更新。例如 Windows「复制到副本」会生成
> `新建文本文档V.2026_0706_163543 - 副本.txt`，再次 move 会更新为
> `新建文本文档V.2026_0706_163550.txt`（自动去掉 ` - 副本` 与旧时间戳）。
> 这同时也自愈了此前错误产生的「双版本」文件名（如 `…V.xxx - 副本V.yyy`）。
